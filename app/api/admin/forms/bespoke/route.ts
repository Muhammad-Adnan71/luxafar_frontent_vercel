...
export async function POST(req: NextRequest) {
  try {
    ...
    await prisma.$transaction(async (tx) => {
      const draggedQuestion = await prisma.bespokeQuestion.findUnique({
        where: {
          id: sourceId,
        },
      });
      await tx.bespokeQuestion.update({
        where: {
          id: sourceId,
        },
        data: {
      ...
      await tx.bespokeQuestion.updateMany({
        where: {
          AND: {
            NOT: {
              id: sourceId,
            ...
            sortId: {
              gte: Math.min(
                draggedQuestion?.sortId as number,
                destinationIdSortId
              ),
              lte: Math.max(
                draggedQuestion?.sortId as number,
